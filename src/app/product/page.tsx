...
.product-hero { padding: 160px 3rem 6rem; }
.product-section { padding: 8rem 3rem; }
.product-section-sm { padding: 6rem 3rem; }
.hero-counters { display: grid; grid-template-columns: repeat(3,1fr); gap: 1.5rem; max-width: 600px; margin: 0 auto 2.5rem; }
.pipeline-flow { display: flex; gap: 0; align-items: stretch; max-width: 1200px; margin: 0 auto; position: relative; }
.pipeline-item { flex: 1; background: #0F1409; border: 1px solid rgba(200,245,62,0.08); padding: 2rem 1.5rem; text-align: center; position: relative; }
.pipeline-connector { width: 40px; display: flex; align-items: center; flex-shrink: 0; }
.detect-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 1rem; max-width: 1100px; margin: 0 auto; }
.tech-grid { display: grid; grid-template-columns: repeat(2,1fr); gap: 1.5rem; max-width: 900px; margin: 0 auto; }
.table-scroll-wrapper { overflow-x: auto; -webkit-overflow-scrolling: touch; width: 100%; max-width: 900px; margin: 0 auto; }

@media (max-width: 1024px) {
  .detect-grid { grid-template-columns: repeat(2, 1fr) !important; }
}

@media (max-width: 840px) {
  .pipeline-flow { flex-direction: column !important; gap: 1rem !important; }
  .pipeline-connector { display: none !important; }
  .hero-counters { grid-template-columns: 1fr !important; max-width: 320px; }
  .tech-grid { grid-template-columns: 1fr !important; }
}

@media (max-width: 639px) {
  .product-hero { padding: 120px 1.25rem 4rem !important; }
  .product-section { padding: 4rem 1.25rem !important; }
  .product-section-sm { padding: 3.5rem 1.25rem !important; }
  ...
  return (
    <>
      ...
      <div style={{ background: '#060A04', color: 'white', fontFamily: 'Inter,system-ui,sans-serif' }}>
        ...
        {/* HERO */}
        <section className="product-hero" style={{ position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: '900px', margin: '0 auto' }}>
            ...
            <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.5rem,7vw,6rem)', fontStyle: 'italic', fontWeight: 900, lineHeight: 1.05, margin: '0 0 1.5rem' }}>From Pixel to Prescription<br />in 4.2 Seconds.</h1>
            {/* Counters */}
        ...
        {/* PIPELINE TIMELINE */}
        <section className="product-section" style={{ background: '#0A0E07' }}>
          ...
          <div ref={pipeRef} className="pipeline-flow">
            {pipeline.map((p, i) => (
        ...
        {/* DETECTION GRID */}
        <section className="product-section" style={{ background: '#060A04' }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.5rem,5vw,4rem)', fontStyle: 'italic', fontWeight: 900 }}>WHAT CropGuard SEES</h2>
          </div>
        ...
        {/* SUPPORTED INPUTS */}
        <section className="product-section-sm" style={{ background: '#060A04' }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: '3rem' }}>
        ...
        {/* COMPARISON TABLE */}
        <section className="product-section-sm" style={{ background: '#0A0E07' }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2rem,4vw,3.5rem)', fontStyle: 'italic', fontWeight: 900 }}>WHY CropGuard WINS</h2>
          </div>
          <div className="table-scroll-wrapper">
            <table style={{ width: '100%', minWidth: '600px', margin: '0 auto', borderCollapse: 'collapse' }}>
              <thead><tr>
                {['FEATURE', 'TRADITIONAL SCOUTING', 'CropGuard AI'].map((h, i) => (
        ...
        <section style={{ background: '#C8F53E', padding: 'clamp(3rem, 6vw, 6rem) clamp(1.25rem, 4vw, 3rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontSize: '15vw', fontWeight: 900, color: 'rgba(6,10,4,0.07)', whiteSpace: 'nowrap', pointerEvents: 'none' }}>CropGuard</div>
